export default function AuthorInfo({ formData, setFormData }: any) {
  return (
    <div>
      ...
      <input type="text" pattern="/^\d{2}-\d{2}-\d{4}$/" placeholder="Author's birth date" value={formData.authorBirthDate} onChange={(event) => setFormData({ ...formData, authorBirthDate: event.target.value })} />
    </div>
  );
}